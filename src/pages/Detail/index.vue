        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(skuSaleAttr, index) in spuSaleAttrList" :key="index">
  ...
  methods: {
    changActive(spuSaleAttrValue, spuSaleAttrValueList) {
      ...
      spuSaleAttrValueList.forEach((item) => {
        item.isChecked = "0";
        // if(spuSaleAttrValue.id == item.id) spuSaleAttrValue.isChecked = '1'
      });
      //点击的那个为高亮
    ...
    async addShopCar() {
      //发送请求
    try {
      await this.$store.dispatch('detail/addOrUpdetaShopCar',{skuId:this.$route.params.id,skuNum:this.skuNum})
      //路由跳转
