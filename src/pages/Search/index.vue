            ...
            <div class="navbar-inner filter">
              <ul class="sui-nav">
                <li :class="{active:isOne}" @click="changOrder(1)">
                  <a>综合<span v-show="isOne">{{UPorDOWN}}</span></a>
                </li>
                <li :class="{active:isTwo}" @click="changOrder(2)">
                  <a>价格<span v-show="isTwo">{{UPorDOWN}}</span></a>
                </li>
              </ul>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    attrInfo(attr,attrValue){
      // console.log('父组件',attr,attrValue);
      //整理数据
      let props = `${attr.attrId}:${attrValue}:${attr.attrName}`;
      //判断props是否已经存在该数据，不存在在添加面包屑
